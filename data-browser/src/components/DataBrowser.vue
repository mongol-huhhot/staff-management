...
import PagedAgGridCard from './helper/PagedAgGridCard.vue'
...
console.log("configStore?.MAIN_CONFIG?.genre==", configStore?.MAIN_CONFIG?.start_end)
...
const currentSqlTag = computed(() => dataType.value ?? 'get_work_report')

const columns = computed(() => {
  return configStore?.MAIN_CONFIG?.columns?.[currentSqlTag.value] ?? []
})

const START_DATE = computed(()=> configStore?.MAIN_CONFIG?.start_end?.[currentSqlTag.value].START_DATE)
const END_DATE = computed(()=> configStore?.MAIN_CONFIG?.start_end?.[currentSqlTag.value].END_DATE)

const dateRange = ref([START_DATE.value, END_DATE.value])
...
const fetchData = async () => {
  ...
  // console.log('sqltag====', currentSqlTag.value, p)

  const genreObj = configStore?.MAIN_CONFIG?.genre?.filter(el=>el.sql_tag === currentSqlTag.value)

  dateFormat.value = 'YYYY-MM-DD'
  if(genreObj && genreObj.length > 0) {
    if( genreObj?.[0].data_type === 'monthly' )
