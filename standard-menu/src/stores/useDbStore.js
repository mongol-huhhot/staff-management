...
export const useDbStore = defineStore('dbStore', () => {
  ...
  const dbAccessWithMultiTags = async (params = {}, options = {}, SQL_PATH = null, URL = null) => {
    ...
    // console.log("dbAccessWithMultiTags raw resp111111===", resp,unwrapMultiTagResult(resp))
  ...
  const normalizeLoginResponse = (resp) => {
    const loginData =
      resp?.result?.['authenticate.login']?.result?.[0]?.result?.[0] ||
      resp?.result?.login?.result?.[0]?.result?.[0]
    ...
    const content =
      typeof loginData.content === 'string'
        ? JSON.parse(loginData.content)
        : loginData.content || {}
    ...
    return {
      ...
      user: {
        ...
        user_type: content.user_type || null,
        roles: content.roles || [],
        content,
      },
      raw: resp,
  ...
  const login = async (sqltag, params = {}, options = {}, SQL_PATH = null) => {
    ...
    const normalized = normalizeLoginResponse(resp)

    if (normalized.token) {
      saveToken(normalized.token, !!options.persist)
    }
