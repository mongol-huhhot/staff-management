...
window.appConfig.MAIN_CONFIG = {
  debug_mode: true,
  app_key: "role_management",   // mtb_item_categoryテーブルのcategory_codeになります。
  ...
  grid: {  // 左に一覧を表示したい場合はこれを追加。個人用個人情報登録にこれがないです
    ...
    condition:{ enabled: '' }, // こちらに必要なすべて条件項目を定義、デフォルト値も入れる！！！
    // ✅ columns を関数化
  ...
  tab2sqltag_list: { // 機能別の定義
    roles: { // mtb_categoryテーブルのsub_category_codeの値になります
      label: 'ロール編集',
      ...
      condition: { role_code:null ,enabled: 'active'}, // selectのときの条件にrole_codeのみが必要です。こちらに必要なすべて条件項目を定義、デフォルト値も入れる！！！
    },
  },
};
