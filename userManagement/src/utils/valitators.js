...
export async function userIdExists(value) {
  if (!value) return true;
  const exists = await useDataStore().checkExistingOfUserid({ checkval: value });
  return !exists; // true: OK, false: already exists
}
 ...
 */
export const rules = {
    required: (value) => !!value || "必須項目です", //Required field
    passwordCheck: (value,check) => (!!value&&value==check) || "パスワードが違います", //password wrong
    email: (value) => isEmail(value) || "メールアドレスではない形式です", //Invalid email
    phone: (value) => phoneNumber(value) || "Invalid phone number", //Invalid phone number
    minLength: (min) => (value) =>
        (value && value.length >= min) || `${min}文字以上入力してください`, //Minimum length is ${min} characters
};
