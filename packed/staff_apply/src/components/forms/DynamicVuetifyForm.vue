...
function isControlAvailable(controlName) {
  const control = controls.value?.[controlName]
...
const showNewRequestAction = computed(() =>
  isControlAvailable('newRequest')
)

const showDraftAction = computed(() =>
  isControlAvailable('draftSave')
)

const showSubmitAction = computed(() =>
  isControlAvailable('submit')
)

const showTerminateAction = computed(() => {
  return (
    canTerminate.value &&
    controls.value?.delete?.disabled !== true
  )
})
...
const showActionBar = computed(() => {
  return (
    props.showSubmit &&
    (
      showNewRequestAction.value ||
      showDraftAction.value ||
      showSubmitAction.value ||
      showTerminateAction.value
    )
  )
})

const newRequestButtonLabel = computed(() => {
  const configuredLabel =
    controls.value?.newRequest?.label
  ...
  return (
    labels[currentRequestStatus.value] ||
    '編集を開始'
  )
})
...
const submitButtonLabel = computed(() => {
  ...
  const requestType =
    ...
    (formData.value?.record_id ? 'update' : 'create')
...
>
  <div class="form-action-bar__inner">
    ...
    -->
    <div
      v-if="showTerminateAction"
      class="form-action-bar__danger"
    >
      <v-btn
        type="button"
    ...
    <!-- 通常操作・主操作 -->
    <div class="form-action-bar__main">
      <v-btn
      ...
      <v-btn
        ...
        type="submit"
        class="form-action-button form-action-button--primary"
        color="primary"
        variant="flat"
...
.form-action-bar__danger {
  display: flex;
  align-items: center;
}

.form-action-bar__main {
  display: flex;
  align-items: center;
