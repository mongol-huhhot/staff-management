...
// const fileKey = computed(() => props.meta.documentType)
...
/*
 * 一時URLはフォームデータへ含めず、
 * UploadImageWrapper内だけで保持する。
 *
 * {
 *   "file-uuid": "https://temporary-preview-url"
 * }
 */
const previewUrls = ref({})

/*
 * 非同期処理中にmodelValueが切り替わった場合、
 * 古い結果による上書きを防止するための連番。
 */
let previewLoadSequence = 0

const normalizedModelFiles = computed(() => {
 ...
 * watchの監視値。
 * オブジェクト全体のdeep監視を避け、
 * file_kindとfile_uuidの変更だけを監視する。
 */
const modelFileSignature = computed(() =>
  normalizedModelFiles.value
    .map(
      file =>
        `${file.file_kind}:${file.file_uuid}`
    )
    .sort()
    .join('|')
)

const findModelFile = (field) => {
  const targetKind =
    `${props.meta.documentType}_${field}`

  return normalizedModelFiles.value.find(
    file => file.file_kind === targetKind
  ) || null
}

const imguuid = (field) => {
  return findModelFile(field)?.file_uuid || null
}

const imgUrl = (field) => {
  const fileUuid = imguuid(field)

  if (!fileUuid) {
    return null
  }

  return previewUrls.value[fileUuid] || null
}
...
const normalizeUploadResult = (result) => {
  ...
  if (!response?.file_kind || !response?.file_uuid) {
    console.warn(
      'アップロード結果にfile_kindまたはfile_uuidがありません',
      result
    )
...
/**
 * file_kindをキーに画像情報をマージする。
 *
 * 同じfile_kind:
 *   既存データへ新しいfile_uuidを上書き
 *
 * 新しいfile_kind:
 *   配列へ追加
 *
...
const mergeUploadedFiles = (
  ...
  for (
    const file of Array.isArray(currentFiles)
      ? currentFiles
      : []
  ) {
    if (!file?.file_kind) {
    ...
    mergedByKind.set(file.file_kind, {
      ...(mergedByKind.get(file.file_kind) || {}),
      ...file,
    })
  }

  for (
    const file of Array.isArray(updatedFiles)
      ? updatedFiles
      : []
  ) {
    if (!file?.file_kind || !file?.file_uuid) {
      continue
    }
  ...
  return Array.from(mergedByKind.values())
}
  ...
  try {
    ...
    for (let i = 0; i < validComponents.length; i++) {
      ...
      try {
        ...
        if (result) {
          console.log("uploadimagewrapper.vue saveresult================",result)
    ...
    if (successCount > 0) {
      const updatedFiles = results
        .filter(item => item.success)
        .map(item => normalizeUploadResult(item.result))
        .filter(Boolean)
      ...
      /*
       * ImageUploaderを経由して
       * DynamicVuetifyFormのupdateFieldへ通知する。
       */
      emit('update:modelValue', mergedFiles)
      ...
      for (
        let i = 0;
        i < childComponents.value.length;
        i++
      ) {
...
const handleDeleted = async (event) => {
  const fileUuid = event?.uuid

  if (!fileUuid) {
    return
  }

  const ok = await fileStore.softDeleteFile(
    fileUuid,
    {
      loading: true,
      loadingText:
  ...
  delete nextPreviewUrls[fileUuid]
...
const loadPreviewUrls = async () => {
  ...
  const fileUuids = [
    ...new Set(
      normalizedModelFiles.value.map(
        file => file.file_uuid
      )
    ),
  ]
   ...
   */
  if (
    currentSequence !== previewLoadSequence
  ) {
    return
  }
