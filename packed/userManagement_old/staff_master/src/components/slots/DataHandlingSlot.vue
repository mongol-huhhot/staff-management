<!-- 
 DataHandlingSlot.vue
    This component provides a slot for handling data operations such as fetching, saving, updating, deleting, and exporting data.
    It uses Vue's Composition API to manage state and handle events.
    It also includes methods for exporting data to Excel and PDF formats.
    The component is designed to be reusable and can be integrated into various parts of the application.
    It accepts props for SQL tags, data, parameters, and success messages, and emits events for data operations.
    by chne, 2025/07/01
-->
<script setup>
...
// data: The data to be handled, can be an object or an array
// sqlPath: Optional SQL path for operations
// sqlTags: An object containing SQL tags for various operations (select, insert, update, delete, upsert, multiupsert)
// params: Additional parameters for operations
// successMessages: An object containing success messages for each operation
// fixedItems: An array of items that should not be included in the payload
// steps: An array of steps for onboarding or guides
// loading: A boolean to indicate if an operation is in progress
// The component emits events for data operations and provides methods for fetching, saving, updating, deleting
// and exporting data. It also includes a resetForm method to clear the form data.
...
async function fetchData(payloadOverride = null) {
    try {
        ...
        console.log("payload.value===", payload.value, result);
        if (result) {
...
// Reset form
const resetForm = () => {
    console.log("resetForm::::: payload.value===",payload.value)
    Object.keys(payload.value).forEach((key) => {
        if (key !== 'content') payload.value[key] = "";
        else payload.value[key] = {};
    });
};
