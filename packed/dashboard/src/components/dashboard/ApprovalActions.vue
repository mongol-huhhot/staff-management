<template>
  ...
  <v-dialog v-model="approveDialog" max-width="480">
    <v-card>
      ...
      <v-card-text>
        <p class="mb-4">
          {{ selectedRow?.staff_name || selectedRow?.user_name }}様の申請中（submitted）の申請を
...
async function approveAllRequests() {
  ...
  const ok = await dataStore.saveData('approve_staff_requests', {
    ...
    approved_by: 'admin', // TODO: ログインユーザーに置き換える
