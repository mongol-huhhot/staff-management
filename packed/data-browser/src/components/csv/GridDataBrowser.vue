...
const headers = computed(() => {
    ...
    if (props.rowData && props.rowData.length > 0) {
        return Object.keys(props.rowData[0]).map(el => ({field: el, headerName:el, style:{}, click:false }));
    }
    return [];
});
...
function csvDownload() {
    ...
    downloadCSV( props.rowData, null, filename, props.filters,)
}
...
<template>
    ...
    <slot name="content" v-if="props.rowData&&props.rowData.length > 0">
